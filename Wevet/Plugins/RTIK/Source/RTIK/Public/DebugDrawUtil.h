...
public: 
	...
	static void DrawBone(UWorld * World, USkeletalMeshComponent& SkelComp, FCSPose<FCompactPose>& Pose, const FCompactPoseBoneIndex& ChildBone, 
		const FLinearColor& Color = FColor(0, 255, 255), 
		float Duration = -1.0f, 
		float Thickness = 1.0f);

	static void DrawBoneChain(UWorld* World, USkeletalMeshComponent& SkelComp, FCSPose<FCompactPose>& Pose, const FCompactPoseBoneIndex & ChainStartChild, const FCompactPoseBoneIndex& ChainEndParent,
		const FLinearColor& Color = FColor(0, 255, 255), 
		float Duration = -1.0f, 
		float Thickness = 1.0f);
};
