...
void FAnimNode_HumanoidLegIKKneeCorrection::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	{
#if ENABLE_IK_DEBUG
		UE_LOG(LogRTIK, Warning, TEXT("Knee Correction - HipFootAxisPre Normalization Failure : %s"), *FString(__FUNCTION__));
#endif 
	...
	{
#if ENABLE_IK_DEBUG
		UE_LOG(LogRTIK, Warning, TEXT("Knee Correction - HipFootAxisPost Normalization Failure : %s"), *FString(__FUNCTION__));
#endif
	...
	{
#if ENABLE_IK_DEBUG
		UE_LOG(LogRTIK, Warning, TEXT("Knee Correction - FootToePre Normalization Failure : %s"), *FString(__FUNCTION__));
#endif
	...
	if (!FootToePost.Normalize())
	{
		...
		UE_LOG(LogRTIK, Warning, TEXT("Knee Correction - FootToePost Normalization Failure : %s"), *FString(__FUNCTION__));
#endif
...
#if ENABLE_IK_DEBUG_VERBOSE
		UE_LOG(LogRTIK, Warning, TEXT("Knee correction -- rotation Axis normalization failure : %s"), *FString(__FUNCTION__));
#endif
	...
	const FTransform NewShinTransform = FAnimUtil::GetBoneCSTransform(*SkelComp, Output.Pose, Leg->Chain.ShinBone.BoneIndex);
	...
	if (bEnableDebugDraw)
	{
		...
		FDebugDrawUtil::DrawBoneChain(
			World, 
			*SkelComp, 
			BasePose.Pose,
			Leg->Chain.FootBone.BoneIndex, 
			Leg->Chain.HipBone.BoneIndex,
			FColor(255, 0, 0));
		...
		FDebugDrawUtil::DrawLine(
			World, 
			ToWorld.TransformPosition(HipCSPre),
		...
		FDebugDrawUtil::DrawLine(
			World, 
			ToWorld.TransformPosition(HipCSPost),
